...
if (provider !== 'local') {
  throw new Error(`STORAGE_PROVIDER inválido: ${provider}. Esta versão produção usa armazenamento local persistente para Hostinger.`);
}

function resolveMediaRoot() {
  if (process.env.MEDIA_ROOT) return path.resolve(process.env.MEDIA_ROOT);
  if (process.env.NODE_ENV === 'production') {
    const home = process.env.HOME || process.env.USERPROFILE;
    if (!home) throw new Error('HOME não disponível. Defina MEDIA_ROOT para um caminho persistente.');
    return path.join(home, 'fabiano-reis-media');
  }
