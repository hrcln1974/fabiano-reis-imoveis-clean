#!/usr/bin/env node
/**
 * Smoke test funcional da API (produção).
 *
...
async function main() {
  ...
  try {
    ...
    // produção — extensão executável disfarçada de imagem (MIME válido, extensão perigosa).
    ...
    // produção.1 — conteúdo real do arquivo precisa corresponder ao MIME declarado.
    ...
    // produção.1 — arquivo vazio com MIME válido também é recusado.
    ...
    const leadId = Number(lead.data?.id);
    const excluirSemSessao = await api('DELETE', `/api/leads/${leadId}`);
    check('exclusão de lead sem sessão retorna 401', excluirSemSessao.status === 401, excluirSemSessao.status);
    const leadExcluido = await api('DELETE', `/api/leads/${leadId}`, { auth: true });
    check('lead pode ser excluído', leadExcluido.status === 200, JSON.stringify(leadExcluido.data));
    const leadsAposExclusao = await api('GET', '/api/leads', { auth: true });
    check('lead excluído não aparece mais na listagem', !(leadsAposExclusao.data?.leads || []).some(l => Number(l.id) === leadId));
    const leadInexistente = await api('DELETE', `/api/leads/${leadId}`, { auth: true });
    check('exclusão de lead inexistente retorna 404', leadInexistente.status === 404, leadInexistente.status);
    ...
    // produção — health check da aplicação + banco, sem vazar segredos.
