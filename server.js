...
// Wrappers mantidos para não alterar as rotas existentes (compatibilidade interna).
...
// Inicializar o SQLite persistente da Hostinger.
function initDb() {
  ...
  db.serialize(() => {
    ...
    // CRM — CRM funcional: clientes, histórico de interações e visitas.
    ...
    // produção — migração incremental: orçamento estimado informado pelo lead.
    ...
    // produção — índices para as consultas realmente usadas pelo catálogo,
...
app.use('/api', async (req, res, next) => {
  try { await dbReady; next(); } catch (err) {
    ...
    res.status(503).json({ erro: 'Banco de dados indisponível. Verifique o armazenamento persistente da aplicação.' });
  }
});
...
function healthHandler(req, res) {
  ...
  db.get('SELECT 1 AS ok', [], err => {
    ...
    res.status(bancoOk ? 200 : 503).json({
      ...
      versao: APP_VERSION,
      banco: { conectado: bancoOk, driver: db.driver || db.mode || 'desconhecido', arquivo: db.databaseFile || null },
...
app.delete('/api/leads/:id', verificarCorretor, (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ erro: 'ID de lead inválido' });

  db.run('DELETE FROM leads WHERE id = ?', [id], function(err) {
    if (err) {
      console.error('Erro ao excluir lead:', err.message);
      return res.status(500).json({ erro: 'Erro ao excluir lead' });
    }
    if (Number(this.changes || 0) === 0) return res.status(404).json({ erro: 'Lead não encontrado' });
    return res.status(200).json({ mensagem: 'Lead excluído com sucesso', id });
  });
});

// =====================================================================
// CRM — CRM FUNCIONAL + WHATSAPP
...
app.get('/api/crm/resumo', verificarCorretor, (req, res) => {
  const queries = [
    ...
    ['visitas_hoje', "SELECT COUNT(*) AS total FROM visitas WHERE data_visita = date('now')"]
  ];
...
// produção: dados de contato centralizados em um único lugar do servidor e
...
// =====================================================================
// produção — Slugs, páginas de imóvel, depoimentos, indicadores e sitemap
...
// produção: SITE_URL fixa o domínio oficial em canônicas/sitemap (evita canônicas
