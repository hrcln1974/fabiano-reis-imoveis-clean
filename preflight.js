#!/usr/bin/env node
/**
 * preflight.js — Validações pré-deploy produção
 * 
...
console.log(`${BLUE}  PREFLIGHT 1.0.0 — Validações de Deploy${RESET}`);
console.log(`${BLUE}═══════════════════════════════════════${RESET}\n`);
...
if (nodeMajor > 22 || (nodeMajor === 22 && nodeMinor >= 5)) {
  ...
  errors.push(`Node.js 22.5+ necessário (atual: ${nodeVersion})`);
}
...
// ============= CHECK 5: HOSTINGER-ONLY =============
try {
  const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
  const allDeps = Object.assign({}, pkg.dependencies || {}, pkg.optionalDependencies || {}, pkg.devDependencies || {});
  if (allDeps['@neondatabase/serverless'] || allDeps['@vercel/blob']) {
    errors.push('Dependência externa de banco/storage incompatível com esta release');
  } else {
    passes.push('Runtime exclusivamente local/Hostinger ✓');
  }
  if (fs.existsSync(path.join(__dirname, 'vercel.json')) || fs.existsSync(path.join(__dirname, 'api', 'index.js'))) {
    errors.push('Artefato de runtime serverless incompatível com esta release');
  }
} catch (_) {}
...
if (fs.existsSync(dbAdapterPath)) {
  ...
  if (dbContent.includes('node:sqlite') && !dbContent.includes('@neondatabase/serverless')) {
...
if (isProd) {
  ...
  requiredEnvVars.forEach(varName => {
    if (!process.env[varName]) {
      if (varName === 'NODE_ENV' && process.env[varName]) passes.push(`Env var ${varName} ✓`);
      else if (varName === 'JWT_SECRET' && process.env[varName]) passes.push(`Env var ${varName} ✓`);
      else errors.push(`Variável de ambiente ausente: ${varName}`);
    }
  });
} else {
