...
// painel: tema persistente (light/dark) sem depender de framework.
...
async function excluirLead(id){
  ...
  if(!Number.isInteger(leadId)||leadId<=0){alert('Lead inválido. Atualize a página e tente novamente.');return;}
  if(!confirm('Excluir este lead definitivamente? Esta ação não pode ser desfeita.'))return;
  try{
    const res=await apiFetch(`${API_BASE}/leads/${leadId}`,{method:'DELETE'});
    const d=await res.json().catch(()=>({}));
    if(!res.ok)throw new Error(d.erro||'Não foi possível excluir o lead');
    await carregarLeads();
    alert(d.mensagem||'Lead excluído com sucesso.');
  }catch(e){
    console.error('Falha ao excluir lead:',e);
    if(e.message!=='UNAUTHORIZED')alert(e.message||'Erro ao excluir lead');
  }
}
  ...
  try{
    ...
    c.innerHTML=imagens.length?imagens.map(img=>
      `<div class="media-card ${img.principal?'is-main':''}">
        <img src="${escapeAttr(img.arquivo||img.url_externa)}" alt="Foto do imóvel" onerror="this.onerror=null;this.src='/img/placeholder.svg';this.style.opacity='1'">
